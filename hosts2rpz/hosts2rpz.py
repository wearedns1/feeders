#!/usr/bin/env python3
"""hosts2rpz: download a hosts file and render an RPZ zone file."""
...
_HTTP_HEADER_PREFIX = "HOSTS2RPZ_HTTP_HEADER_"
_FEEDER_NAME = "hosts2rpz"
...
class HostsStats:
    ...
    skipped_local_records: int
    invalid_domain_records: int
...
def _normalize_domain(value: str) -> str | None:
    ...
    return f"{wildcard_prefix}{domain.lower()}"
...
def _iter_hosts_domains(hosts_text: str) -> tuple[list[str], HostsStats]:
    total_records = 0
    candidate_records = 0
    skipped_local_records = 0
    invalid_domain_records = 0
    ...
    for line in hosts_text.splitlines():
        ...
        total_records += len(tokens) - 1

        for host in tokens[1:]:
            if host.strip().strip(".").lower() in {"localhost", "localhost.localdomain"}:
                skipped_local_records += 1
                continue

            candidate_records += 1
            domain = _normalize_domain(host)
            if domain is None:
                invalid_domain_records += 1
                continue
            if domain != host.strip().strip(".").lower():
    ...
    stats = HostsStats(
        ...
        skipped_local_records=skipped_local_records,
        invalid_domain_records=invalid_domain_records,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="hosts2rpz")
    parser.add_argument(
        "--config",
...
def main() -> int:
    ...
    run_id = _new_run_id()
    log_timestamp = _bool_env("HOSTS2RPZ_LOG_TIMESTAMP", True)
    log_level = _normalize_log_level(_env("HOSTS2RPZ_LOG_LEVEL", "info"))
    action_success = _env("HOSTS2RPZ_ACTION_SUCCESS")
    action_not_modified = _env("HOSTS2RPZ_ACTION_NOT_MODIFIED")
    action_failure = _env("HOSTS2RPZ_ACTION_FAILURE")
    source = ""
    output_path = ""
    ...
    try:
        source = _required_env("HOSTS2RPZ_SOURCE")
        output_path = _required_env("HOSTS2RPZ_OUTPUT")
        zone_name = _normalize_zone_name(_required_env("HOSTS2RPZ_ZONE_NAME"))
        ttl = int(_env("HOSTS2RPZ_TTL", "300"))
        soa_refresh = int(_env("HOSTS2RPZ_SOA_REFRESH", "300"))
        soa_retry = int(_env("HOSTS2RPZ_SOA_RETRY", "60"))
        soa_expire = int(_env("HOSTS2RPZ_SOA_EXPIRE", "604800"))
        soa_minimum = int(_env("HOSTS2RPZ_SOA_MINIMUM", "300"))
        sinkhole = _normalize_sinkhole(_env("HOSTS2RPZ_SINKHOLE", "CNAME ."))
        add_wildcard = _bool_env("HOSTS2RPZ_ADD_WILDCARD", False)
        header_comment = _normalize_header_comment(_env("HOSTS2RPZ_HEADER_COMMENT"))
        use_etag = _bool_env("HOSTS2RPZ_USE_ETAG", True)
        timeout = int(_env("HOSTS2RPZ_TIMEOUT", "30"))
        max_redirects = int(_env("HOSTS2RPZ_MAX_REDIRECTS", "3"))
        user_agent = _env("HOSTS2RPZ_USER_AGENT", "hosts2rpz/1.0")
    ...
    try:
        hosts_text, response_etag = _read_source(
            source,
            timeout=timeout,
    ...
    _log(
        ...
        min_level=log_level,
        duration_ms=_elapsed_ms(step_started_at),
        bytes=len(hosts_text.encode("utf-8")),
    )

    step_started_at = time.perf_counter()
    try:
        raw_domains, stats = _iter_hosts_domains(hosts_text)
    except Exception as exc:
        return fail(
            "hosts_failed",
            duration_ms=_elapsed_ms(step_started_at),
    ...
    if not domains:
        return fail(
            "hosts_failed",
            duration_ms=_elapsed_ms(step_started_at),
            message="hosts source contains no usable domains",
        )
    _log(
        "hosts_processed",
        log_timestamp,
        ...
        skipped_local_records=stats.skipped_local_records,
        invalid_domain_records=stats.invalid_domain_records,
